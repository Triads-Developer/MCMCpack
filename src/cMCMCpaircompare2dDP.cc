...
void MCMCpaircompare2dDP_impl (rng<RNGTYPE>& stream,
  ...
  for (unsigned int iter = 0; iter < tot_iter; ++iter){
    ...
    if (verbose > 0 && iter % verbose == 0) {
		...
		Rprintf("Minimum gamma acceptance rate: %7.3d ", gamma_accept_min);
		Rprintf("\nMean gamma acceptance rate:    %7.3d ", gamma_accept_num/gamma_accept_denom);
		Rprintf("\nMaximum gamma acceptance rate: %7.3d ", gamma_accept_max);
		Rprintf("\n\n");
